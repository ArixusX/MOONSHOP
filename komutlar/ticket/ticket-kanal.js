  ...
  if (args[0] === "ayarla") {
    ...
    if (kanalbelirle)
      return message.channel.send(
        `Mesajı göndereceğim kanal zaten ayarlı: l!ticket-kanal sıfırla`
      );
    ...
    message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setDescription(
          `Mesajın kanalı başarıyla ayarlandı: l!ticket-mesaj gönder yazarak ticket kanal açma mesajını atabilirsin.`
        )
    );
  }
  ...
  if (args[0] === "sıfırla") {
    ...
    message.channel.send(
      new Discord.MessageEmbed()
        ...
        .setFooter( "LUISA PLUS+ / Discord'da Yeni Devrim!", client.user.avatarURL())
        .setDescription(
          `Mesajın kanalı başarıyla sıfırlandı: l!ticket-kanal ayarla #kanal`
        )
    );
  }
};
